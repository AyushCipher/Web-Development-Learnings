...
router.post("/createTodo", createTodo); // (path set kro, controller k specific function se map kro[handler])
router.get("/getTodos", getTodo);
router.get("/getTodos/:id", getTodoById); // The :id acts as a placeholder for a dynamic value. When a request is made to this route, whatever value is passed in place of :id will be accessible in your handler function.
...
// In the context of backend development, a route is a defined URL pattern that the server listens to, and 
// when that pattern is matched in an incoming request, the server performs a specific action or returns a particular response.
// Routes are used to handle requests for different resources or actions in a web application. 
// They are commonly part of web frameworks (like Express in Node.js, Flask in Python, or Django in Python) and are associated with HTTP methods (GET, POST, PUT, DELETE, etc.).
