...
async function publishEvent(routingKey, message) {
  ...
  // succeeded (see createPost) - would turn a working post creation into a
  // reported failure, exactly the kind of misleading response found live-
  // testing this: MongoDB had the new post, but the API said "Error
  // creating post". Publishing to RabbitMQ is fire-and-forget messaging
  // for OTHER services to react to later - it failing shouldn't undo or
  // hide that the primary write already succeeded.
  if (!channel) {
