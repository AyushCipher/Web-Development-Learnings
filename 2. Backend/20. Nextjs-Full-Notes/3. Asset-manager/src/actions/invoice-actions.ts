"use server";
// Server Actions for generating and retrieving invoices.
// createInvoiceAction is called automatically from
// src/actions/payment-actions.ts (recordPurchaseAction) right after a
// purchase is committed - buyers never trigger it directly. getInvoiceHtmlAction
// is what src/app/api/invoice/[id]/route.ts calls to serve the stored HTML
// back to the browser.
...
// Called from src/app/api/invoice/[id]/route.ts. The route handler already
// confirmed *a* session exists; the ownership/admin check happens here so
// the route handler stays a thin wrapper and the "who can see this invoice"
